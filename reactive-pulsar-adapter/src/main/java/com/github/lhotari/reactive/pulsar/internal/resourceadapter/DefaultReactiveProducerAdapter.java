...
import static com.github.lhotari.reactive.pulsar.internal.adapter.AdapterImplementationFactory.adaptPulsarFuture;
...
class DefaultReactiveProducerAdapter<T> implements ReactiveProducerAdapter<T> {
    ...
    private Mono<Tuple2<DefaultProducerCacheKey, Mono<Producer<T>>>> createCachedProducerKeyAndMono() {
        return Mono.fromCallable(() -> {
            ...
            ProducerBuilderImpl<T> producerBuilder = (ProducerBuilderImpl<T>) producerBuilderFactory.apply(
                pulsarClient
            );
            DefaultProducerCacheKey cacheKey = new DefaultProducerCacheKey(
                pulsarClient,
                producerBuilder.getConf().clone(),
    ...
    private <R> Mono<R> usingUncachedProducer(Function<Producer<T>, Mono<R>> usingProducerAction) {
        return Mono.usingWhen(
            ...
            producer ->
                Mono.using(
                    () -> producerActionTransformer.get(),
                    transformer ->
                        usingProducerAction.apply(producer).as(mono -> Mono.from(transformer.transform(mono))),
                    Disposable::dispose
                ),
            this::closeProducer
        );
    ...
    private <R> Mono<R> usingCachedProducer(Function<Producer<T>, Mono<R>> usingProducerAction) {
        return createCachedProducerKeyAndMono()
            .flatMap(keyAndProducerMono -> {
                ProducerCacheKey cacheKey = keyAndProducerMono.getT1();
                Mono<Producer<T>> producerMono = keyAndProducerMono.getT2();
                return producerCache.usingCachedProducer(
                    cacheKey,
                    producerMono,
    ...
    private <R> Flux<R> usingCachedProducerMany(Function<Producer<T>, Flux<R>> usingProducerAction) {
        return createCachedProducerKeyAndMono()
            .flatMapMany(keyAndProducerMono -> {
                ProducerCacheKey cacheKey = keyAndProducerMono.getT1();
                Mono<Producer<T>> producerMono = keyAndProducerMono.getT2();
                return producerCache.usingCachedProducerMany(
                    cacheKey,
                    producerMono,
                    producerActionTransformer,
                    usingProducerAction
